...
### COMMON TIME EVIDENCE OS｜AI 如何进入规划决定

`Evidence OS` 不是一套接管城市的自动平台，而是六个受限规划角色组成的证据流水线。它把任务书 `agent.1—agent.6` 所需的总体统筹、生态、场景、公共空间、文化和运营成果，转成同一条可追踪的 `输入 → AI 限定任务 → 人工决定 → 空间动作 → 观测 → 退役` 链。六个角色不共享个人画像，不读取非公开资料，也不能批准建设、采购、法定路权、生命安全或公共权利决定。

| 受限角色 | 允许输入 | 只允许产出的规划对象 | 必须人工签发 | 禁止与失败退出 |
| --- | --- | --- | --- | --- |
| `SENSE` 问题登记 | 清权公开资料、匿名现场计数、设备校准与缺失值 | 问题卡、时空基线、证据 ID 和 `UNKNOWN` 清单 | 问题业主、数据责任人、受影响群体代表 | 不采人脸/车牌/精确轨迹；来源或同意不明即删除并回到人工记录 |
| `COMPARE` 反事实 | 同地点同时间的非 AI 基线、试点记录、群体分布 | 差异表、置信范围、失败/无效结果和单位成本分母 | 独立评测者、专业核验者 | 不只报平均值、不把相关性写成因果；样本/阈值未注册即不得比较 |
| `MATCH` 空间匹配 | 已签问题卡、空间类型、专业硬门、维护能力 | `共享 / 轻改 / 可逆补点 / 必要建设` 候选及替代方案 | 产权/资产管理人、规划与专业团队 | 不生成法定红线、确定拆建或伪精确工程量；硬门未过则 `HOLD` |
| `SIMULATE` 时段协商 | 固定规则、容量/噪声/无障碍候选阈值、非 AI 对照 | 时窗冲突、排队/到达情景和人工接管演练 | 运营者、交通/消防/无障碍专业人员 | 不控制法定路权或生命安全系统；任一严重事件触发物理停用 |
| `EXPLAIN` 公共转译 | 已清权原文、模型/数据卡、术语表和版本 | 双语说明、纸数同源导引、可理解性测试和勘误单 | 内容权利人、文史/专业编辑、现场人工服务 | 不代替专业诊断/裁决；无法溯源、纠错或提供非数字等价服务即撤回 |
| `AUDIT` 采用账本 | 前五角色的只读快照、签署、成本、申诉和事件记录 | G0—G8 状态、到期提醒、差异审计和退役证明 | 独立主评、申诉责任人、采用决定人 | 不静默改阈值、不自动续期；证据过期、冲突未披露或退出资金缺失即 `RETIRE` |
...
| 阶段 | 最小空间与服务包 | AI / 非 AI 对照 | 必须形成的证据 | 不过门的处理 |
| --- | --- | --- | --- | --- |
| D0—15 授权与登记 | 只选既有城市侧空间；核对消防、无障碍、安静边界；一张问题墙、一张权利牌 | `SENSE` 登记清权问题卡；非 AI 为纸表、人工访谈和现有服务 | 产权/运营授权、RACI、平面/照片、清权目录、退出责任 | 任一授权或生命安全门缺失：不布置、不招募 |
| D16—30 双基线 | 可移动桌椅、纸质资料架、人工服务台、清晰进出路径 | `COMPARE` 只比较同一任务的人工/纸质基线与候选数字辅助 | 工作日/周末与昼/夜原始记录、缺失值、两人复核、阈值共签 | 无非 AI 基线或受影响群体未参与：回到 D0 |
| D31—60 可逆试用 | 无固定施工；静态导引、离线材料、可拔电设备、可见急停与人工接管 | `MATCH/EXPLAIN` 给出授权内容与候选路线；人工主持并可完全离线 | 每次任务完成/误解/纠错、无障碍与非数字同效、噪声/劳动/OPEX | 误导、排除、安静或劳动负担恶化：当日暂停并恢复人工基线 |
| D61—75 独立复算 | 场地保持原状，第三方只读访问冻结快照 | `AUDIT` 生成差异清单；独立方按预注册规则复算，不用供应者自评分 | 数据哈希、回避声明、群体差异、事件与申诉闭环、成本簿 | 无法复算或阈值被更改：试点无效并撤场 |
