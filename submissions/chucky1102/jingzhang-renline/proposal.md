...
**实地踏勘(2026-08-24)。**人类账号所有者沿线实地踏勘一次:13号线四站(西直门—大钟寺—知春路—五道口)加一期徒步段,西直门K0至学清路K8,16张精选照片存于 `assets/site-photos/` 并逐张登记原始文件名、UTC时间与经纬度(全部照片保留原始GPS与时间EXIF,不含可识别人脸,未进入围挡或铁路用地)[source:FIELD-SURVEY-20260824]。踏勘将三条在地锚点由G1升至**G3(现场可观察)**:遗址公园建成与开放状态、13号线拆分工程施工现场、觉生寺文物本体与周边整治;清华园车站旧址本次未及,如实停留G1。三项现场观察补充如下——其一,公园二期已于2026-08-06开放(南段大运村足球场—西直门、北段清华东路—北五环,与一期连成约9公里),但廊道呈**建成段、在建段、未整治段三态并存**:红色跑道绿道与施工围挡在大钟寺段一墙之隔,大钟寺—知春路间仍有瓦砾未整治段——"二期开放"是过程性事实而非完成态,本方案的绿脊主张相应定位为"在这条不均质的已开放廊道上做增量与缝合",不把已建公园表述为本方案成果。其二,知春路侧13号线轨道旁拆分工程基坑正在施工,该段"强割裂"判定现场成立。其三,绿道上观察到外卖骑手正常穿行,T1"绿脊自动接驳与无人配送走廊"场景所依赖的配送生态在现场真实存在。其四,把68张照片的GPS点连成轨迹叠到本包几何上后,**实测轨迹局部位于 provisional 边界以西**——这不是测绘结论,而是临时边界相对真实廊道存在偏移的直观登记,与"官方红线公布后整包重算"的既有声明互为印证(数据冲突登记为证据的做法致谢 Grady10086/railweave,独立执行)。

![清华园车站旧址一代站房,2023年修缮后(公开来源照片:李汉宗/Lhzss8,2023-10-03,CC BY-SA 4.0,via Wikimedia Commons;等比缩放,非踏勘影像)](assets/reference-photos/qinghuayuan-station-1908-building-2023.jpg)

上图为**公开来源照片**、非本包踏勘影像:清华园车站旧址本次踏勘未及,为补视觉缺位收录 Wikimedia Commons 开放许可照片一张(作者与许可见图注,登记于 [source:WIKIMEDIA-QHY-OLDHOUSE-2023]);按证据纪律,公开来源影像属 G1 支撑,QHY 锚点等级**不因此改变**,G3 仍须踏勘者现场可观察。

![踏勘轨迹图:68张照片GPS点自西直门至学清路连续覆盖K0-K8,实测轨迹局部位于provisional边界以西,直观呈现临时边界偏移](assets/figures/survey-track.png)
...
![三重点区首层界面展开:每段沿街开间标出 AI 撤除后仍在的实体要素(人工问询岗、纸质登记、无障碍入口、实体警示牌等),与 SEB 节点等价路径逐项对位;现状观察等级 G3/G3/G1 如实标注](assets/figures/street-interface.png)
...
**北京AI原点社区(原点站,约104.3公顷)。**定位:世界级AI创新生态的日常发生地——不是园区,是"生活着的模型社区"。空间结构:西侧清华东AI原点研发区(校城融合界面),东侧五道口智能消费区(青年夜间经济),原点广场(里程K5·人字交点)居中锚定 [data:geometry/public_space.geojson#PS-PLZ-3]。建筑更新:以既有楼宇更新+少量新建为主,重点是把**经协商确定的优先界面段**沿街首层改为开发者友好界面(工位、路演、市集;范围以与权属方协商结果为准)。交通:五道口站接驳径与成府路缝合街打通东西。AI场景:智能体市集、开发者街面、夜间经济智能治理。实施风险:高校边界开放需校方共建机制,人流高峰治理需与轨道运营协同。
...
**三重点区概念体量与首层界面台账(概念建议,现状列以2026-08-24踏勘照片为据)。**逐区登记"现状首层界面观察→概念首层策略→体量包络原则",观察缺位处如实标注:

| 重点区 | 首层界面现状(踏勘观察) | 概念首层策略 | 体量包络原则 |
| --- | --- | --- | --- |
| 大钟寺(钟声站) | 餐饮零售门脸连续、非机动车占道;大体量玻璃办公与文物一街之隔;紧邻大片拆迁平整地(照片 k1-street-frontage / k1-dazhongsi-officeblock / k1-renewal-clearance) | 存量门脸整理+文化展示界面向钟声广场开放 | 文物周边严控高度体量,保护范围内只做低强度退让 |
| AI原点社区(原点站) | 五道口站桥洞瓶颈与商业综合体界面并存,过街人流密集(照片 chengfu-stitch) | 沿街首层全部改为开发者友好界面(工位/路演/市集) | 既有楼宇更新为主、少量新建,校城界面渐进开放 |
| 众智园(登顶站) | **本次未入内观察**,仅外围见大规模在建(照片 north-construction),现状列如实缺位 | 全栈验证设施的首层开放展廊(概念) | 战略留白优先,包络待官方控规与专项条件 |

台账只登记"观察到什么"与"概念上主张什么",不含体量测算;三区体量数值一律待官方控规条件 [depth:three_key_area_detailed_design] [source:FIELD-SURVEY-20260824]。
...
**试点基线与复审计划(逐场景,均为 design_target,非任何主体的承诺)。**回应评审"为 T1-T3 与高敏感公共服务场景补充试点基线与复审计划"的要求,对三个测试验证场景与两个高敏感民生场景(卡6无障碍副驾、卡7养老诊疗辅助)逐一登记六要素——基线采样、责任角色、硬停止条件、恢复证据、无障碍与非智能路径测试、独立复核;全部纳入折返协议90天公开复审,基线数据以聚合形式随复审结论公布,重申校验通过≠现场有效:

| 场景 | 基线采样(黄灯期) | 责任角色(Accountable) | 硬停止条件(立即转红) | 恢复证据 | 无障碍与非智能路径测试 | 独立复核 |
| --- | --- | --- | --- | --- | --- | --- |
| T1 绿脊接驳/配送 | 前30天建立基线,后按周聚合:接管次数与中位时长、近失数、投诉率、行人冲突事件;仅脱敏轨迹与聚合数 | 运营方安全负责人(具名入折返档案) | 任一伤害事故;单周近失超基线上限 | 缺陷归因+整改+受控场地复测通过,连续两复审期合格 | 轮椅/婴儿车通行实测;既有步行与公交路径保持不削减 | 年度第三方安全审计;保险方事故复核 |
| T2 具身测试场 | 每场次事件日志(出界/急停/干预);月度聚合 | 场地伦理委员会召集人 | 机器人越出围合边界;任何人身伤害 | 事故报告公开+整改验证+空场复测 | 封闭场地不涉公众路径;对外直播打码抽查 | 伦理委员会+保险方双复核 |
| T3 市政AI体检 | 与人工巡检双轨90天对照:误报率/漏报率/工单一致率 | 市政运营方巡检负责人 | 漏报导致设施实际故障 | 参数校准+对照组复测达标 | 人工巡检制度保持,不因AI减编 | 主管部门抽样复检 |
| 卡6 无障碍副驾 | 任务完成率/中途放弃率按障碍类型分组;真人测试须含视障、听障、认知障碍用户(知情同意招募) | 运营方无障碍负责人 | 错误指引造成人身危险 | 缺陷修复+同组用户复测通过 | 非智能等价=人工客服,接通时限纳入采样;认知可达文本分级 | 无障碍组织参与式评审 |
| 卡7 养老诊疗辅助 | 预诊提示与执业医师最终判断一致率(仅聚合)、误报致扰投诉 | 合作医疗机构医务管理部门 | 出现任何绕过医师直接给出诊疗结论的输出 | 全流程审计+规则修复+医务复验 | 大字/语音/家属代理三通道;线下挂号路径保持 | 医学伦理委员会年度审查 |

上表所有阈值遵循既有判据纪律:不预填虚构数字,一期先测量基线、再定值并公开;采样周期与复审期对齐(90天),数据最小化边界按各卡片已登记口径执行。
...
**协议与相邻标准的互操作已声明,并且可被机器拒绝。**0.3.0 新增 `interop` 块,登记与 lqqk7 发布的《服务等价基准 SEB》(CC-BY-SA-4.0,Issue #2549)的对照关系。此处纠正一处易错映射:外部对照把互转轴称作 `gate_id` 并假定它在卡片层,但本协议卡片层没有该字段,只有 `gate`(当前验证阶梯位置,承担等级排序,它才是互转轴)与 `ascent_grade`(**已达成**的证据等级)。二者共用 G0-G3 值域而语义不同,把后者当互转轴会让一张只有 G0 纸面证据的卡片认领 L3 开放等级——即 R4「纸面证据不得冒充现场证据」在互操作层的同型违例。新增规则 R5 即拒绝此类映射,配一正一负样例,`node visual/assets/run-switchback-validation.js` 可复算 [data:visual/assets/switchback-validation-receipt.json]。**v1.6-v1.7 期间本包未采用 SEB 任何字段;v1.8 起采用其节点 schema 组件(仅五项必填字段,见下一小节),但仍不认领任何 SEB 开放等级(L0-L4)**;对照表与 R5 是给同时采用两套标准的方案用的。这一采用恰好落在 R5 所警告的位置上,因此说明其规避方式:SEB 的 `gate_id` 被用在**节点**上(十驿的当前闸门,全部 G0),本协议的 `gate`/`ascent_grade` 用在**卡片**上,两者不共享对象也不互相推导;十处节点没有一处据此认领任何开放等级。R5 禁止的是"以已达成证据等级冒充互转轴",本采用不构成该违例,可由两份文件各自的校验器分别复算。

**AI 关闭之后,这条带子还剩什么。**本方案把"非智能等价路径"从条款升格为**准入前提**:一处公共节点若说不清 AI 关闭后人怎么把事办成,它就还没有资格上 AI。这一条与折返协议同源——折返协议要求场景证明自己能被退回,才允许爬升;这里要求节点证明自己在 AI 缺席时仍然成立,才允许接入。两者是同一条纪律在时间轴与空间轴上的两面。

**十驿逐点声明,采用 SEB 节点 schema。**为使该主张可被外部机器检验而非自说自话,K标十驿 K0-K9 逐点按 lqqk7《服务等价基准 SEB》v0.5.0 的**节点 schema 组件**(规范内组件标识 `node_schema`)声明五项必填字段:`ai_off_path`(AI 关闭后的等价路径)、`human_handoff`(可被找到的人工接管角色)、`gate_id`、`operating_mode`、`responsible_role`,登记于 `visual/assets/seb-adoption.json` [data:visual/assets/seb-adoption.json]。选择该组件而非自造字段,是因为 SEB 对其中两项写了机器判据而非自然语言劝告:`ai_off_path` 命中"扫码、小程序、线上办理、注册账户"等 14 项同一系统词即判定不成立——理由是等价路径若仍依赖同一系统,比较的是一条路的两个入口而不是两条路;`human_handoff` 必须命中 13 项角色词之一,只写机构名不算,因为使用者站在现场要找的是一个人而不是一个单位。十处节点全部通过该判据(禁用词命中 0、角色词缺失 0、枚举违规 0、字段缺失 0),自检结论随文件登记,任何人可取 SEB 规范原文的词表复算。十处 `ai_off_path` 逐点不同而非套用同一句:等价路径由该点的实际空间条件决定——K0 是里程零点碑与取阅架上的纸质折页,K2 是站口实体导视与公告栏张贴的体检结论,K4 是把 AR 历史层的同一内容并置为桥面固定展板使不持设备者获得同等信息,K9 是实体围挡、地面标线与警示牌界定的测试场边界。同一句话抄十遍等于没有逐点设计。

采用范围严格限定:**只采节点 schema 一个组件**,不采评分口径、等级定义、判定规则、AI 权限边界与双联交接台账;**不认领任何 SEB 开放等级**。采用文件按可核验四要素登记上游坐标(canonical path、schema_version 0.5.0、exact commit `949f6b94`、内容 SHA-256 `adb930c3…`)与 CC-BY-SA-4.0 署名。此处也是一次互采登记:SEB 发布文本在机制谱系中署名折返协议(Issue #1119)为其体裁与开放方式的来源先例,本文件是反向的组件级采用;两个方向都是自愿登记,不构成任何一方对另一方的背书。

**六类画像 × AI 关闭后的完整任务链。**包容性不能停在"设有无障碍与人工兜底"这类总括句,它必须落到"谁、要办成什么事、AI 没了怎么办、在哪儿办"。逐画像登记如下,每条任务链均锚定到具体 K 标节点:

| 画像 | 关键任务 | AI 关闭后如何完成 | 节点锚点 |
| --- | --- | --- | --- |
| ①AI研究员/博士生 | 查实验设施开放时段、找深夜食堂与住处 | 学院路开放段的时段与范围以实体导视及门岗公示牌为准;原点广场取阅架常备生活折页与年度纸本名录 | K7、K5 |
| ②创业者与独立开发者 | 租工位、报名路演、上架智能体、纠纷申诉 | 市集设纸质摊位登记表,受理与结果均出纸质回执;现场设人工仲裁席,不经由任何账户体系 | K5 |
| ③产业工程师 | 申请测试场时段、确认测试边界与安全规则 | 测试场边界由实体围挡、地面标线与警示牌界定,不依赖联网提示;时段以纸质预约登记与现场引导岗确认 | K9 |
| ④社区原住民与长者 | 知悉施工影响、日常办事、就医预诊、投诉 | 市政体检结论与施工影响改以站口公告栏定期张贴,附纸质留言与申诉表格;卡7 保持线下挂号路径与大字/语音/家属代理三通道;人工客服为兜底而非转接同一系统 | K2、K5 |
| ⑤青少年与家庭访客 | 参加科普活动、获取 AR 历史层内容 | 科普驿实体展板与纸质活动单;AR 历史层的同一内容以桥面固定图文展板并置,不持设备者获得同等信息而非简版 | K8、K4 |
| ⑥国际访问者与会议客商 | 寻路、获取大会议程、多语言信息 | 南门户纸质全带折页地图(含十驿位置与步行时长);钟声广场多语言实体展签与可借还的离线语音导览机(不需个人身份信息);大会设纸质议程与现场引导岗 | K0、K1、K9 |

![公共剖面:AI 设施全部旁置于步行净线之外,撤除后沿街首层界面、人工窗口、步行净线与纸质规则四项均不受影响;尺寸为概念控制目标,非现状测量](assets/figures/public-section.png)

**三项包容性指标已登记,且刻意保持 unknown。**`non_ai_task_completion_target_ratio`(AI 关闭后关键任务完成率,设计目标值 1.0)[metric:non_ai_task_completion_target_ratio]、`no_login_task_completion_target_ratio`(不以账户或个人标识为前置的完成率,设计目标值 1.0)[metric:no_login_task_completion_target_ratio]、`equity_access_burden_gap_target_minutes`(非智能路径相对 AI 路径的额外耗时,设计目标值 ≤10 分钟,按六类画像分组报告而非取合并中位数——合并会掩盖长者与无智能手机使用者的实际负担)[metric:equity_access_burden_gap_target_minutes]。三项在 `metrics.json` 中 `status` 均为 `unknown`、`value` 均为 `null`:**目标值不是测量结果,本包没有任何一处节点在运行(十驿 `gate_id` 全部 G0),观察用户数为零**。分母口径一并写明——完成、撤回、技术故障、其他非完成四类必须一并报告,删除任一类该次测量作废;这条纪律取自 SEB 评分口径组件的公开条文,本包未采用该组件,仅借其分母定义避免日后用删样本的方式抬高完成率。参与式核验(居民、长者、障碍人士、商户与一线运营人员)尚未开展,应在试点授权与运营主体落实后进行,本方案不把它当作已完成的工作陈述。
...
三点补充口径:其一,**近期可启动项**——①②④⑤⑨五项可不待官方红线与控规条件先行开展**前期研究、权属与文保协商及轻量试点**(存量协商、既有公园一期机制与桥下空间活化均有现行路径);实质实施仍以各项目所列权属、文保、用地移交等前置条件落实为准;其二,**退出条件**——所有项目遵守"依赖条件未满足即维持现状,不留半拆工程"原则,地标类项目均含降级方案(如原点碑可先以数字荣誉墙运行,不建实体也成立);其三,成本量级仅分"存量改造/公共空间/新建"三带表达相对高低,不做投资测算(测算属 blocked 状态,需专业前置)。实施政策建议(均为概念建议):存量改造适用城市更新条例路径;高校界面开放采用"校地共建协议+运营让利"机制;测试场景适用监管沙盒;留白区建立"全球AI机构总部预留+触发式启动"机制。公众参与:每期设方案公示与社区共创工作坊,更新不驱逐原则写入项目前置条件。
...
**爬升等级:证据也以迭代换爬升。**为避免"说得越多显得越真",本方案给主张与场景统一标注证据等级,等级只能靠新证据"爬升"、不能靠修辞:G0=方案主张(仅设计判断)→G1=有公开来源支撑→G2=包内可复算(GeoJSON/metrics可独立验证)→G3=现场可观察→G4=多方签注→G5=独立复核通过。当前诚实盘点:全部面积与网络指标为 **G2**(EPSG:4548可复算,但受provisional边界限制);在地锚点中,遗址公园建成开放、13号线拆分工程、觉生寺文物本体三条经2026-08-24实地踏勘升至 **G3**(现场可观察,16张GPS照片登记于 `visual/assets/field-survey-register.json` [source:FIELD-SURVEY-20260824]),清华园车站旧址本次未及、如实停留 **G1**;12张场景卡均为 **G0-G1**(概念设计,其中T1-T3须在黄灯期依次通过三级验证门方可升至G3);容积率等待确认控规条件为 **G0**(unknown,不作主张)。每次爬升的条件与折返协议的90天复审绑定,升级与降级同样公开(证据分级思路致谢 knqiufan/listening-line-jingzhang 的E0-E5证据梯,按其 CC-BY-SA-4.0 许可署名引用,本方案作 RENLINE 化独立转译)。
