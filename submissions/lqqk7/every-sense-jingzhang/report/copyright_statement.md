...
- Source Han Sans SC is used under the SIL Open Font License 1.1 and is subset-embedded in the PDF outputs. From v9.8 the package additionally redistributes one glyph subset of the same family: a woff2 subset embedded as a base64 data URI inside `visual/assets/cjk-webfont.css`, so that Chinese text in the HTML deliverables renders in a browser with no CJK font installed. That subset is a Modified Version under the OFL, is renamed to `ESJZ Sans Subset` as clause 3 requires, keeps the upstream copyright and licence name records verbatim as clause 2 requires, and ships alongside the licence text in `visual/assets/font-license-ofl.json`; its upstream release URL, hashes and subsetting parameters are registered in `visual/assets/cjk-webfont-provenance.json`. No other font file is redistributed in any form. A full-book font audit (pypdf, 2026-08-13) confirms the four drawing PDFs embed only SourceHanSansSC Regular/Medium/Bold subsets with zero non-OFL entries; earlier builds had picked up system-font subsets through the browser print pipeline's font-stack fallback, which this version removes with a build-time normaliser and a repeatable audit script.
...
The ledger below restates, asset by asset, what the sections above declare in prose. It covers all 156 file entries registered in `manifest.json`. Every row is compiled from a record that already exists inside this package — no new rights fact, tool record, or licence claim is introduced here — and the last column of each table names the file the entry was compiled from, so any row can be checked word for word against its source. Tables are written in Chinese with bilingual headings; where a field is not registered anywhere in the package, the entry reads「包内未登记」rather than a guessed value, and all such gaps are listed in the final section.
...
| --- | --- | --- | --- | --- |
| A 方案正文与结构化文件 | 14 | 申报智能体原创生成 | Claude Fable 5 via Claude Code（含 Claude Opus 子代理）；v1.0 阶段为 GPT-5.6 Sol via Codex | 本文件 · `agent.json` |
...
| F 离线展示与随包代码 | 57 | 原创手写，自含离线；含 SEB 许可授予文件、四个历史版本快照、OSM 精简快照两片与肌理读数、三维主脊视图与第三方组件 three.js r160 及其许可正本、法律接口登记与审查过程归档 | SEB 判据工具零依赖（仅 Node 内置模块）；展示层含唯一第三方组件 three.js r160（MIT，许可正本 `three-license.json`） | 本文件 · `proposal.md` |
...
| `simulation.json` | 由包内既有结构化登记逐条离线复算编制 | 同 `proposal.md`；固定 seed 离线合成推演，不调用在线模型、不接入真实设备、无真实参与者，判定按文件内 `decision_rules` 复算 | 同上；三项读数为离线演练结果，不代表现场实测，不构成效果承诺 | `simulation.json` `method` 与 `derived_from` |
...
| `visual/assets/font-license-ofl.json` | 随包收录的 SIL Open Font License 1.1 许可正本与重建哈希，覆盖本包内一切来自思源黑体的字面数据：四册 PDF 内嵌的三字重子集，以及 v9.8 起随包收录的中文网页字体子集 | OFL 1.1 允许再分发许可文本本身。v9.8 起本包分发一份字体数据（`visual/assets/cjk-webfont.css` 内以 base64 data URI 内嵌的 woff2 子集），按 OFL 1.1 第 2 条随附本许可正本与版权声明，按第 3 条保留名条款改名后分发 | 本文件 §四字体台账 |
| `visual/assets/cjk-webfont.css` · `cjk-webfont-provenance.json` | v9.8 新增：中文离线字体子集及其溯源登记。子集自 Adobe 官方 2.005R 发布物 `09_SourceHanSansSC.zip` 内的 `SourceHanSansSC-Regular.otf` 用 fontTools 生成，只保留本包四份 HTML 与七个注入文案的脚本中实际出现的 1,323 个非 ASCII 字符，输出 woff2 198,300 字节，以 base64 data URI 内嵌于一份 CSS，两份报告页与两份展示页以相对路径共用这一份，全包只存一份字面数据。溯源文件载上游发布物 URL 与 SHA-256、源字体 SHA-256、子集参数全表、按 OFL 保留名条款所作的更名清单、unicode-range 例外的八个码位及其理由，以及覆盖断言的十九项读数。v9.9 因正文新增两个此前未出现的汉字（「她」「悔」）按同一套参数与同一份上游源字体重出子集，字符集 1,321 → 1,323、woff2 198,180 → 198,300 字节，子集参数、更名清单与许可履行方式一字未改，新旧产物哈希与本轮重出理由逐项登记于溯源文件的 `revision_history` | Source Han Sans SC 2.005R，SIL Open Font License 1.1，© 2014-2025 Adobe，保留名 `Source`。子集属 OFL 定义的 Modified Version，族名已改为 `ESJZ Sans Subset`、PostScript 名改为 `ESJZSansSubset-Regular`，各语言族名记录一并改名，不使用保留名；name ID 0 版权声明与 name ID 13／14 许可声明及许可 URL 在子集内逐字保留，以履行 OFL 1.1 第 2 条。上游发布物内 `LICENSE.txt` 的 SHA-256 与 `font-license-ofl.json` 已登记的 `plain_text_sha256` 实测相同，溯源链自上游发布物到包内许可正本闭合 | `visual/assets/cjk-webfont-provenance.json`；本文件 §四字体台账 |
...
| --- | --- | --- | --- | --- | --- |
| 图纸 PDF（`drawings/*.pdf`） | Source Han Sans SC（Regular/Medium/Bold 三个子集） | 本地安装副本；许可正本随包收录于 `visual/assets/font-license-ofl.json`（OFL 1.1 允许再分发许可文本） | SIL Open Font License 1.1 | 子集嵌入 PDF；不作为独立资产随包分发。每册每字重只嵌入一份子集，全册各页共用（逐页渲染原本让每页各带一份分页子集，v8.1 按字形标识空间取并集合并为一份，合并前后逐字形核对轮廓与度量全等）。经全册字体审计（pypdf 遍历页面与 XObject 资源；审计脚本自 v10.0 起对每个字体字典**双取**名称——`/BaseFont` 缺失时改取 `/FontDescriptor` 的 `/FontName`，Type0 复合字体下钻至 `/DescendantFonts[0]` 的描述符后再取，2026-08-30 四册复跑）：**非 OFL 嵌入字体程序 0 条，非 OFL Type3 描述符名 0 条**。此处如实登记本版查获并消除的一处口径缺陷与一处实体缺陷。**口径缺陷**：Type3 字体按 PDF 规范不设 `/BaseFont`，其名称只出现在描述符的 `/FontName` 里，而 v9.9 及更早的审计只读 `/BaseFont`，因此四册合计 20 条 Type3 条目对旧脚本全部不可见，据此写出的「非 OFL 字体条目为 0」与 `pdffonts` 的实测直接矛盾。**实体缺陷**：双取后查明，A3 中文册第 2 页的「灰」与第 15 页的「粗」「略」三个字符不在页源内嵌的思源黑体子集内，打印管线回退到 macOS 系统字体 PingFang SC 并将其转为 Type3 轮廓路径，构成三条越出已清权 OFL 范围的描述符名（该三条不随包分发任何字体程序）。**处置**：按 `unicode-range` 追加一份仅含该三个字符的同族思源黑体子集（Regular/Medium/Bold，取自与既有子集同源的本地安装副本），既有字符的字体匹配结果不可能被改变，据此重出 A3 中文册——`pdftotext` 全文与改动前逐字相同、页数 16 与页幅 420.20 × 297.01 mm 未变，Type3 条目由 12 条降为 11 条且全部指向思源黑体，四册 Type3 计数为 3／0／11／5，四册 `/FontFile*` 与 Type3 `/CharProcs` 流的现行读数为 35／13／103／47 条（A3 中文册由 104 降为 103：三条 PingFang Type3 条目各含一个 `/g0` 空路径与一个实绘字形，重出后由两条思源黑体 Type3 条目取代——第 2 页一条带一个实绘字形、第 15 页一条带两个实绘字形，条目数由 12 降为 11、`/CharProcs` 流由 71 降为 70，实绘字形总数 3 不变），`pdffonts` 四册再无任何 PingFang 条目。此前版本的四册亦曾因同一条字体栈回退路径混入系统字体子集，已于更早版本清除并加入构建期兜底；本版补齐的是该兜底未覆盖的缺字回退与审计脚本的读取口径。v9.8 为腾出字节额度对四册内的 JPEG 图像流作了一次重编码（质量 76，仅替换 `/DCTDecode` 图像流），字体程序不在处理范围内：四册全部 `/FontFile*` 与 Type3 `/CharProcs` 流（v9.8 时点 35／13／104／47 条）重编码前后原始字节逐条相同，`pdffonts` 字段集与 `pdftotext` 全文摘要亦前后相同 | 本文件 §Generated assets and software |
| 离线展示页与报告页（`visual/index{,.en}.html`、`report/proposal{,.en}.html`、`scene.css`、`spine3d.css`） | 中文字面由随包子集 `ESJZ Sans Subset` 承担，排在每一条字体栈首位；其后整条保留原有系统字体栈（`"Source Han Sans SC"`、`"Noto Sans CJK SC"`、`-apple-system`、`"PingFang SC"`、`"Microsoft YaHei"`、`Arial`、`sans-serif`），标题字族 `Georgia` 与西文排印标点仍由运行环境解析 | 子集源自 Adobe 官方 2.005R 发布物；许可正本随包收录于 `visual/assets/font-license-ofl.json`，溯源登记于 `visual/assets/cjk-webfont-provenance.json` | SIL Open Font License 1.1 | v9.8 起以 base64 data URI 内嵌于 `visual/assets/cjk-webfont.css` 随包分发，四份页面以相对路径共用同一份。页面仍不发起任何远程字体请求。v9.7 及更早版本不携带任何字面，中文在未安装 CJK 字体的环境里退化为方框，该缺陷由 v9.8 修复 | `visual/assets/cjk-webfont-provenance.json`；本文件 §三 F 类对应行 |
...
| 品牌落地字体策略 | 衬线标题、无衬线正文、等宽数据三分工，现阶段三种分工均由 Source Han Sans SC 一族承担（Medium/Bold/Regular） | Source Han Sans SC（SIL OFL 1.1）；商业字体候选待逐项清权 | 逐项核验授权，未清权字体不进入公开成果 | 品牌图版（`brand-system.png`）的字体策略区块已与清权后事实对齐：三种分工同出思源黑体一族（Medium/Bold/Regular，SIL OFL 1.1），与四册 PDF 实际嵌入的三个子集一一对应；Georgia、Helvetica Neue、SF Mono 等商业字体在图版上明示为未来深化候选，须逐项取得商用授权后方可引入。本包携带字体程序的载体有两处：四册 PDF 的子集嵌入（见首行），以及 v9.8 起随包分发的 `visual/assets/cjk-webfont.css`（见次行）；两处同出思源黑体一族、同按 SIL OFL 1.1 履行，图版（PNG/SVG）不含任何字体数据 | `proposal.md` 品牌章与「版权与授权」段 |

**一致口径（v9.8 更正）：** 本包分发的字体数据只有两类，两类都是思源黑体的字形子集，都按 SIL OFL 1.1 履行随附义务：其一为四册 PDF 内的子集嵌入，其二为 v9.8 新增的 `visual/assets/cjk-webfont.css`，后者以 base64 data URI 内嵌一份 woff2 子集，按 OFL 保留名条款改名为 `ESJZ Sans Subset` 后分发。v9.7 及更早版本此处写作「全包不以独立资产形式再分发任何字体文件」，该句在当时成立，自 v9.8 起不再成立，故按事实更正而不保留旧句。除上述两类外，其余交付物（触觉纸模 SVG、短片、封面、图版）仍不含任何字体数据，依赖运行环境本地字体。
