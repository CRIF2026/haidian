...
区域协同以“公开议题入口—双向资源清单—受控联合任务—可核验成果—知识库回流”为基本链条。下表仅明确本方案建议的协作对象类型、输入输出和回流位置，不表示相关地区、机构或部门已同意合作。具体责任主体、数据权限、经费和实施时序须经有权主体依法确认。

| 协作对象（概念建议） | 京张侧输入 | 协作侧输入 | 建议联合任务 | 可核验输出 | 回流与责任边界 |
|---|---|---|---|---|---|
| 北纬社区协作接口 | 场景责任卡、公众解释模板与纠错测试规程 | 自愿参与团队的产品问题、公开说明和合成测试样例 | 从一个明确的服务失败出发，依次开展A1规则共测、A2解释与人工转介验证、A3整改复测 | 带版本的差距清单、复测记录和退出回执 | 经独立复核后回流A1；由中关村科技服务翼提供可复用方法。团队意愿、授权或维护责任缺失时不启动；不表示已有合作 |
| 未来科学城相关创新节点 | 城市治理问题清单、场景责任模板和人工复核规则 | 公开科研成果、基础模型测试方法和专家能力 | 高风险场景桌面推演与失效注入 | 测试记录、问题清单和修订建议 | 经授权的非敏感结果回流A1治理知识库；未明确数据权限或责任人即停止 |
| 怀柔科学城相关创新节点 | 公共服务需求、可解释性与公众沟通检查表 | 公开科学工具、测量方法与科普转化经验 | 模型结果可理解性与公众展示验证 | 解释模板、公众问题集和人工转介条件 | 回流A2公众复核与导航界面；未通过无障碍和公众可理解性测试不进入现场 |
| 北京经济技术开发区相关产业节点 | 场景开放清单、合规自测模板和退出协议 | 企业产品说明、公开标准实践和脱敏测试条件 | 企业服务Agent的合规沙盒与交付验收演练 | 合规差距清单、退出记录和可复用工具包 | 回流A3企业服务与场景市场，并由A1承担合规共测复核；不形成认证、采购或准入承诺 |
| 京津冀城市治理与产业节点 | 可移植的场景卡、安全闸门和评估指标 | 异地服务条件、差异化法规/流程和运行反馈 | 小规模可逆复现与跨域差异校验 | 适用性边界、本地化修订项和停止条件 | 经A1复核后更新知识版本，由中关村科技服务翼输出已标注适用边界的标准包 |

每项联合任务建议使用独立任务卡，记录议题来源、数据权限、责任人、版本、人工复核结论、停止条件和回流去向。仅有通过授权、安全、公共价值和可复现性检查的成果可进入知识库；其余成果保持隔离、整改或退出。 [source:AGENT-TASKBOOK] [depth:overall_spatial_structure]
...
公众服务序列建议形成“规则公开入口—政策推演与受控测试—专家复核—结果说明—申诉与退出”五个连续空间环节。普通访客首先进入不采集个人信息的规则说明区；只有经授权的测试主体才能进入受控测试区；涉及权利、资源配置或行政处置的结果须离开机器测试界面，转入有权责任人的独立复核空间。各环节以可识别的门槛、状态标识和人工窗口连接，使规则制定能够被看见、测试状态能够被读懂、责任决定能够被追溯。
...
服务序列建议落实为“普通服务入口—AI用途解释与自主选择—辅助服务—人工复核—休息恢复—异议与退出”。居民无需使用智能手机即可进入普通服务；选择AI辅助前应先看到用途、数据边界和人工替代方式；服务失败或公众提出异议时，路径直接转向人工窗口和申诉节点，不要求用户重新进入数字流程。
...
企业服务序列建议形成“需求目录—准入说明—用途授权—隔离测试—合规复核—成果退出或转化”。场景开放、数据使用和合规辅导分别设置责任窗口，避免由单一平台同时承担申请、测试和认证。未通过测试的项目沿原路径安全退出并完成数据删除、权限回收和问题复盘；通过测试只表明完成约定验证，不自动形成政府认证、采购资格或建设承诺。
...
11个场景采用同一套“问题进入—分流研判—受控测试—人工复核—公众发布—纠错退出—知识更新”协议。协议不是新的软件平台，而是所有运营主体共同遵守的最低责任边界：

| 协议阶段 | 必须说明的事项 | 进入或继续条件 | 暂停、回退与公共回执 |
|---|---|---|---|
| 问题进入 | 问题来源、服务对象、公共价值及是否适合使用AI | 来源可说明，且普通服务通道已经存在 | 来源不明或无公共必要性时不进入；登记拒绝原因 |
| 分流研判 | 数据类别、最小必要范围、风险等级和责任岗位 | 数据用途、保存期限与禁止事项得到确认 | 数据权属或用途不清时转人工；保留分流记录 |
| 受控测试 | 测试范围、评价指标、隔离环境和失败注入方法 | 责任人、测试资源和退出方案已经落实 | 越权调用、严重偏差或无法解释时立即停止并回收权限 |
| 人工复核 | 证据版本、异议事项、专业判断和签发权限 | 有权责任人独立判断，不以模型建议代替履职 | 复核无法完成时退回既有人工流程；记录推翻或修改理由 |
| 公众发布 | 结论适用范围、AI参与程度、责任主体和申诉入口 | 信息可理解、可访问，且不泄露个人或商业秘密 | 解释不完整或救济不可达时不得扩大使用 |
| 纠错退出 | 暂停原因、人工替代、数据删除、权限回收和恢复条件 | 纠正措施完成并经责任主体重新核验 | 向受影响者提供处理进度和可核对回执 |
| 知识更新 | 有效经验、失败案例、规则版本和适用期限 | 来源、责任编辑和复核日期完整 | 过期或有争议内容下架，不直接用于后续自动判断 |

每个场景在深化阶段均应形成一张“八项责任卡”：问题入口、最小数据、智能体权限、责任岗位、人工接管、非数字替代、退出条件和证据回执。任一项缺失，场景只能停留在概念或桌面推演阶段，不进入真实公共服务 [source:AGENT-TASKBOOK] [source:GENERATIVE-AI-MEASURES]。

### 3. 三项测试验证场景

#### 从一次服务失败检验整套治理机制

本方案的原创设计贡献，是将公众纠错权、现场人工服务和知识版本管理安排在同一条空间服务链中。规则研究、公众复核与企业整改分别由A1、A2、A3承接，形成可追查的交接关系。国际框架提供风险与问责的方法参考；本方案具体设计的是这些方法在京张如何被公众使用、被运营人员执行、被专业团队复核，不主张发明通用治理原则。

以下为合成桌面情境，不是现场事件或已实施系统：访客在A2导航界面发现推荐的无障碍通行路径不可用。系统保留原始建议版本，由有人值守的服务点接管；公众无需继续使用AI，也无需提交残障证明即可反馈。实际通行安全由现场人员确认，模型不得自行推断可通行性。

| 环节 | 空间与场景 | 人的职责 | 智能体允许做什么 | 形成的记录与停止条件 |
|---|---|---|---|---|
| 发现与求助 | A2导航站，S09/S05 | 服务人员先协助，登记问题 | 提示人工入口，保存被质疑的版本 | CM-DEMO-001；不采集身份、诊断或轨迹 |
| 局部暂停 | A2公众复核台 | 值守人员暂停被质疑的推荐，维持人工服务 | 标记该条知识暂不可用，不停止无关基本服务 | 暂停范围、原因、人工替代；替代不可达则不恢复 |
| 联合核查 | A1协查与知识库，S03/S04 | 场地责任人与内容复核人分别核实空间条件和信息 | 汇集证据、标出版本差异，不代替签发 | 空间证据缺失时记为证据不足，保持暂停 |
| 修正与复测 | A1规则共测；涉及产品缺陷时转A3 | 内容责任人修正；独立复核人确认测试与服务恢复条件 | 在隔离环境重放合成任务，生成差异清单 | 空间可用、人工可达、复测与签发均满足才可恢复 |
| 回执与更新 | A2服务点及A1版本账 | 向反馈者提供纸质或可访问的处理回执 | 关联旧版本与新版本，注明适用范围 | 记录采纳、未采纳或证据不足及理由；保留再次复核入口 |

公众回执只展示问题、处理状态、理由和复核入口；内部测试日志与权限记录不直接对外公开。现场仍未核实的事项不得因模拟通过而解除暂停。展示页提供同一合成记录的展开入口，不连接任何真实业务系统。 [source:AGENT-TASKBOOK] [depth:overall_spatial_structure]
...
| 试点 | 输入与人工对照 | 故障注入 | 验收记录 | 停止与恢复 |
|---|---|---|---|---|
| S03多智能体协查 | 合成事项、冻结职责表；与人工编制的分流基准比较 | 职责冲突、证据缺失、无权签发 | 分流差异、人工改判理由、签发角色与版本 | 冲突转人工；只有责任厘清、证据齐备且有权人员签发后才能进入下一步 |
| S06场景开放市场 | 合成申请与测试协议；由人工逐项核对准入清单 | 授权过期、用途不符、撤回申请、缺少维护责任 | 拒绝原因、权限撤回和退出回执；不产生认证或采购资格 | 缺项拒绝进入；条件补齐须重新审核，不能自动恢复旧授权 |
| S10社区健康哨兵 | 合成聚合服务需求；与专业人员制定的提示基准比较 | 小样本、异常噪声、误报、人工响应不可达 | 样本充分性、误报与漏报、人工研判及服务转介记录 | 样本不足不判通过；无人工响应即暂停提示；不得输出个体诊断 |

三项试点分别固定输入版本、规则版本、测试负责人、独立复核角色、测试窗口及退出责任。验收应同时报告通过、失败、证据不足的数量，禁止剔除失败样本后只报告成功率。所附规则回放检查仅验证上述门槛能否拒绝构造的错误输入，不评价模型准确率、真实跨部门效率或医疗效果。

首轮资源采用工作量法估算：人工复核工时＝有效测试任务数×每任务复核时间；维护工时另计值守、异常处理和复测；资金需求由工时、经确认的单价、场地和工具费用共同计算。当前不填未经核实的预算、岗位编制或机构承诺。各任务启动前须由授权发起人确认资源、场地、安全、数据和退出责任；任一未落实即暂缓启动。
...
试点验收不能只验证系统在理想条件下能够运行，还应验证错误出现时能够安全停止。建议至少设置四类不涉及真实个人信息的负向测试：

- **越权调用测试：** 模拟缺少授权、授权过期或用途不一致的数据请求，系统应拒绝调用、记录原因并通知数据责任人。
- **误判纠正测试：** 注入可被专业人员识别的错误建议，验证人工能否推翻输出、说明理由并回写纠错记录。
- **跨部门冲突测试：** 设置职责边界或规则解释冲突，系统不得自动仲裁行政责任，应暂停流转并进入联合研判。
- **包容性失败测试：** 模拟语音识别失败、屏幕不可读、用户无智能手机等情形，验证能否立即切换纸质、人工、电话或线下等效服务。

每次测试形成“输入条件—预期拒绝或暂停—责任人—人工替代—恢复条件—公众可见回执”六联记录。测试数据、阈值和结果在正式授权前均为拟议方法，不表述为现场实测成效。
...
在0—6个月近期阶段内，建议先嵌入一轮可撤回的12周验证，不以一次性铺开代替逐段核验：

| 周期 | 核心任务 | 阶段证据 | 未通过时的处置 |
|---|---|---|---|
| 第1—2周 | 锁定问题清单、数据边界、责任岗位和普通服务底线 | 基线登记、责任卡、数据清单 | 缺少责任主体或人工替代时暂缓启动 |
| 第3—4周 | 使用公开、合成或脱敏数据开展桌面回放 | 测试脚本、版本号、差异记录 | 结果不可复现时退回规则整理 |
| 第5—6周 | 在隔离环境开展功能测试与错误注入 | 日志、拒绝记录、权限回收记录 | 发生越权或无法停止时终止该场景 |
| 第7—8周 | 开展小范围公众体验与无障碍走查 | 知情说明、匿名反馈、人工转接记录 | 非数字渠道不可达时不得进入下一阶段 |
| 第9—10周 | 开展跨部门协同、申诉和恢复演练 | 联合研判单、纠错回执、恢复条件 | 责任冲突未解决时保持人工流程 |
| 第11—12周 | 由独立专业人员开展阶段评估 | 继续、整改或退出建议及公开摘要 | 任一治理闸门未通过即停止扩展 |

12周安排属于概念建议。测试对象、参与人数、风险阈值、责任单位和公开范围均须在启动前由有权主体依法确认，不得将桌面回放或模拟结果表述为真实运行成效。
...
建议驾驶舱不直接汇集未经定义的“大数据”，而以三本责任清晰、口径稳定的台账作为最小证据底座：

- **空间账：** 登记边界版本、图层来源、面积复算、服务节点、无障碍路径及其 provisional/official 状态；正式边界更新时保留前后差异。
- **场景账：** 登记11个场景的问题入口、最小数据、风险等级、责任岗位、人工接管、非数字替代、暂停原因和纠错回执。
- **版本账：** 登记规则、模型或知识内容的版本号、适用范围、更新时间、变更原因、公众异议和退役记录。

三本账共同支持“看得见、读得懂、可监督、可纠错”：空间账说明服务在哪里发生，场景账说明由谁负责，版本账说明规则为何变化。任何驾驶舱指标都应能返回原始责任记录；无法追溯来源的数值不得作为扩展试点的依据。
...
| 指标 | 计算口径 | 证据不足与解释边界 |
|---|---|---|
| 纠错闭环率 | 已完成核查、处理和可访问回执的事项数／到期应处理事项数 | 分母为零时不适用；未到期事项单列，不算已解决 |
| 人工接管成功率 | 测试窗口内成功到达人工服务的任务数／触发人工接管的任务数 | 超时、放弃、无法连接均保留；时限由责任主体预先确认 |
| 权限拒绝正确率 | 正确拒绝的无权请求数／预标注无权请求数 | 只说明既定测试集，不代表真实系统无漏洞 |
| 错误提示率 | 人工复核判为错误的提示数／已完成复核的提示数 | 同时披露未复核数，不能用选择性复核推断总体误报率 |
| 证据充分率 | 输入、版本、责任、判断和回执齐备的记录数／纳入评估的记录数 | 样本量及抽样方法须预先确认；齐备不等于内容真实或合法 |

上述为设计评价口径，尚无本地实测基线或绩效结论。阈值须在测试启动前由相关专业与责任主体预先确定并锁定版本。硬性停止规则优先于平均成绩：未授权使用、缺少人工服务或缺少必要复核时，不允许以其他指标较好抵消。 [source:AGENT-TASKBOOK] [depth:metrics_recomputation]
...
图件、网页和文字由 OpenAI Codex 基于仓库公开/清权资料和本方案结构化数据生成。空间体验类图件使用 OpenAI 图像生成模型制作，并在图注中明确标识为 AI 生成概念效果图；边界、面积、交通和指标等证据性图件仍依据本地结构化数据绘制，不以生成图替代测绘或法定规划依据。既有图中文字为栅格化输出；中文离线网页及新增PDF文字使用按SIL OFL 1.1许可嵌入的Noto Sans SC子集，字体许可随包保留。完整生成与许可说明见 `report/copyright_statement.md`。
