...
2. **可停止** — 每个AI场景设置人类责任角色与退出机制，责任人在落地阶段逐场景具名指定
...
> **来源ID别名说明**：任务书在本方案中统一以来源注册表 ID `SRC-2026-0518-AGENT-OPEN-CALL-TASKBOOK` 引用，对应标准清单 `standards.json` 中的批准标准 ID `PROJECT-AGENT-OPEN-CALL-TASKBOOK` 及 site-package 文件 `brief/site-package/agent_taskbook.json`——三者指向同一份「面向全球智能体的开源征集任务书」。正文不再使用旧别名 `AGENT-TASKBOOK`。

核心资料来源：
- **官方公告**：资格预审公告文本，含三层范围（统筹研究范围43.6km²、总体设计范围约11.4km²、重点区域368.4ha）的四至描述 [source:DATA-SRC-OFFICIAL-ANNOUNCEMENT-20260509]
- **任务书**：面向智能体的六大设计任务及十大共创章约 [source:SRC-2026-0518-AGENT-OPEN-CALL-TASKBOOK]
...
**工作重点**：识别海淀高校资源（北大、清华、北航、北邮等十余所高校）与AI产业（中关村软件园、后厂村、智源研究院）的空间关系，分析青年人才（海淀区IT业在岗职工53.5万人、科研业20.0万人 [source:HAIDIAN-YEARBOOK-2021-2024]；高校在校生约35万人（设计假设 ASM-009，基于北京市教育统计与海淀高校分布的粗略推算，非项目范围实测值，不作为正式证据））的日常活动轨迹，提出区域级公共空间与创新服务网络。

### 总体设计范围（约11.4 km²）
以京张遗址公园周边1-2公里为规划设计范围 [data:geometry/site_boundary.geojson#SITE-BOUNDARY-001]。以概念性城市设计深度开展工作，聚焦城市更新、用地布局、公共空间体系、交通慢行网络、市政配套和城市风貌，相关深度不构成控制性详细规划结论 [standard:AGENT-SUBMISSION-DISCLAIMER]。
...
3. **青年人才的需求优先级（设计假设 ASM-010，基于全球创新区案例的可转化经验归纳，非本地调查结论）**：24小时可达设施 > 社交场景 > 运动空间 > 绿地 > 商业配套
...
> 上表为概念性用地功能分区；绿地与开放空间面积以 `geometry/green_space.geojson` 实测 141.8ha 为权威口径（对应绿地率 12.42%），分区面积与 site_boundary 实测 1141.28ha 存在概念归类口径差异，空间复算以 GeoJSON 为准 [data:geometry/land_use.geojson]。
...
- AI算力中心的PUE目标≤1.2（设计目标，参照《北京市发展和改革委员会关于进一步加强数据中心项目节能审查若干规定的通知》（京发改规〔2023〕10号）中「年能源消费量≥2万吨标煤项目PUE值不高于1.2」的门槛 [source:BEIJING-DC-ENERGY-REVIEW-2023]；为概念目标，具体项目需按届时适用的审批要求单独测算，非现状承诺）
- 分布式光伏覆盖公园路灯、公交站顶棚、建筑屋面，年发电量约500万kWh为概念估算值，待能源专业测算确认 [metric:haidian_electricity_kwh]
...
| 绿地率 | 12.42% | 绿地实测141.8ha/设计范围实测1141.28ha [data:geometry/green_space.geojson] | 中等（临时边界） |
| 公共空间占比 | 3.0% | 公共空间实测34.24ha/设计范围实测1141.28ha [data:geometry/public_space.geojson] | 中等 |
...
| 设计范围实测面积 | 11.41 km² | 临时边界EPSG:4548投影实测 [metric:site_area_sqm] | 中等 |
