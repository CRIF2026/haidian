...
summary: "一条以午间自助使用为优先的京张公共路线：设计目标是让使用者无需预约、扫码、登录、专用设备或日常工作人员，仍能依靠 Noonline Route Kit 的实体起点牌、模式卡、导向、短停、返回与状态标记完成行走、停留、判断和离开；AI 只作可选解释，核验、维护与停止机制属于后台责任层。"
...
> **把午间做成一条可以自己使用的城市路线：AI 可以增强一条路，但任何人都不应因为没有手机、没有屏幕、AI 离线或无人陪同，而失去一条可进入、可停留、可判断并可离开的公共路径。**

## 快速审阅入口 / Quick Review Entry

本轮把既有的自助规则收束为一套可读的 **Noonline Route Kit**，不新增任何事实主张。**30 秒**先看一位午间使用者如何从固定起点牌选择“短走、短停或无障碍优先”，再完成“进入—步行—短停—判断—离开”，全程不必打开手机、登录系统或等待工作人员；**3 分钟**再看三区如何分别提供研究旁的公共通行、无设备的信息停留与到达后的自主判断；**15 分钟**最后核对 WP-01—06 与 Phase 0—3 如何在后台处理证据缺失、过期或不可审计时的 HOLD、改线、降级或撤除。所有地点关系、构件和条件仍是概念表达或待核验事项，不构成现状、许可或实施承诺。
...
## 0. 一条不必被管理的午间路线 / A Self-Guided Noon Route

方案的前台不是一项需要报到、预约、扫码、下载应用或由工作人员带领的服务，而是一套让人自行完成判断的空间次序。它先用公共清晰步行带、固定信息、可识别的短停位置与非数字判断标记组织体验；使用者可以在其中自己决定继续、暂停、绕行或离开。

这不是对任何现状路线、遮阴、座椅、饮水、入口、过街或服务人员的确认。它是一个概念使用规则：未来只有在有关条件被授权核验后，才可把相应的实体构件布置到具体位置；条件未具备时，不启动该构件，也不把“自助可用”升级为现场事实。

对日常使用者，AI 是侧置的、可退出的解释层，不负责带路，也不是进入或完成路线的条件。方案中的人工责任也不是沿线日常驻守：它只属于维护、例外处置、停止决定和后续专业核验的后台责任层。换句话说，使用者不需要找到某个人，才能把这条午间路线走完。

## 0.1 南北主线、东西接入与五个自助动作 / Spatial Spine, Cross-Stitches and Five Self-Guided Actions

方案把公开任务书的三大定位放进同一套可读的空间关系，而不把它们停留在口号层面。**南北公共主线**按公开材料所述的北部众智园、中部 AI 原点、南部大钟寺形成一条概念性的阅读次序：在众智园，AI 融合创新带表现为“测试可见、公共清晰带不可被侵占”的研发界面；在 AI 原点，都市 AI 生活体验带表现为不依赖设备的信息、停留与提问；在大钟寺，日常到达、判断、短停和回归城市生活形成可自主完成的收束。百年京张文化带则不是附加造型，而是一条贯穿三个原型的固定公共叙事：连接、维护、责任与恢复。

这条南北主线是 **Level 2 的方向识别与公共叙事规则**，不是对连续可走通道、既有服务带或已获准线路的确认。图中的东西关系同样不是新建道路、桥梁、过街或入口的主张。**东西接入**只描述中关村科技服务翼与小月河场景赋能翼在未来与主线发生的“进入—返回”关系：西翼把专业服务、转化和治理接口带到可公开理解的边界；东翼把生态、日常场景与公共反馈带到同一边界。任何具体接入点只有在公共通行、合法过街、无障碍替代、维护与相应专业条件均被授权核验后才可启动。

![V5.1 概念空间骨架：南北主线、东西接入、三大定位与五个自助动作；不是测绘图、现状路线或实施点位](assets/figures/v5_1-spatial-spine-diagram.svg)

为使这套关系在不用手机时仍可被实际读懂，每一个未来经核验的起点、转折与返回关系都采用同一套 **五个自助动作**。它们是信息与空间的规则，不是对任何实体已存在的声称：

1. **进入 / START**：固定起点图说明可用的概念方向和可随时返回的选择，不以二维码、账户或实时导航作为入口。
2. **步行 / WALK**：重复的编号、符号与文字导向沿公共清晰带给出方向；颜色不能是唯一信息。
3. **短停 / PAUSE**：非数字信息把“可停留、可绕开、可返回”的判断放在同一阅读位置，但不把概念短停写成已有座椅、遮阴或饮水设施。
4. **转向 / TURN**：每个未来经核验的判断点同时呈现继续、改线或返回，而不是把使用者交给算法推荐。
5. **返回 / RETURN**：路线始终能回到明确的日常去向；当通行、维护、无障碍或安全条件失效，实体信息先显示停止、降级或替代关系。

因此，南北主线解决“如何连续阅读三处原型”，东西接入解决“从两翼如何进入并回到日常”，五个动作解决“没有扫码时如何自己走”。它们只在 Phase 0 逐点核验通过后才可落到具体位置；任一条件缺失时，相关关系保持 HOLD，不把概念骨架升级为现场事实。

## 0.2 Noonline Route Kit：第一次看到就能使用的实体工具包 / A First-Use Physical Kit

**Noonline Route Kit 不是一款应用，也不是已安装的标识清单。**它是一套未来可在经核验位置重复使用的实体信息规则，把“自助”从抽象原则变成第一次到达时看得见的选择。使用者先看起点牌，不需要先知道项目名称、下载软件或理解后台工作包；每个构件都用文字、编号和符号共同传达信息，不只依赖颜色。

![V5.2 Noonline Route Kit：未来经核验起点的模式选择、六类实体标记、五个自助动作与路线变更状态；不是现状设施或安装图](assets/figures/v5_2-noonline-route-kit.svg)

Route Kit 由六个概念构件组成，全部属于可撤回、可更新的公共信息层：

1. **KIT-01 起点牌 / Start Panel**：在未来经核验的公共起点同时给出当前位置、可选模式、对应符号、返回规则和当前状态；二维码、账户或实时导航只能在牌后作为可选解释，不能替代它。
2. **KIT-02 模式卡 / Mode Strip**：使用者无需输入个人信息，只需在固定信息中选择 **MOVE 短走**、**PAUSE 短停** 或 **ACCESS 无障碍优先**。它们不是现成路线承诺：PAUSE 只指向未来已核验的停留机会，ACCESS 只在合法连续替代已被核验时出现。
3. **KIT-03 步行标记 / Walk Marker**：沿公共清晰带重复同一模式的编号、符号和文字，让人无需手机也能确认自己仍在所选关系内。
4. **KIT-04 短停信息牌 / Pause Panel**：把继续、绕开和返回放在同一阅读面；它可提示一个未来核验后的停留选择，但绝不把座椅、遮阴、饮水或室内空间写成已存在。
5. **KIT-05 转向与返回牌 / Turn and Return**：在每个未来经核验的判断点并列呈现继续、改线和返回，避免把改变路径的判断藏进手机界面。
6. **KIT-06 状态牌 / Status Card**：只使用四种可读状态：**CONTINUE 继续**、**REROUTE 改线**、**RETURN 返回**、**PAUSE 暂停**。它必须告诉使用者下一步，而不只是显示一条故障说明。

这套工具包为第一次使用者提供一个最小剧本：先在起点选择模式，记住符号与编号；步行时只核对下一枚实体标记；想停时读取短停信息；遇到转折或条件变化时按状态牌继续、改线、返回或暂停。AI 可以在实体标记之后提供理由、活动提示或维护说明，但不收集身份、人脸或个人轨迹，也不成为完成路线的条件。

Route Kit 的最小未来试点不是整条走廊，而是一段经授权的 **“起点—两处判断—一处短停信息—返回”** 关系。Phase 0 必须先核验公共性、合法通行、无障碍替代、实体信息可读性、维护接口和停止权；任一项缺失，相关卡牌不上线或立即撤回。工具包由此让“不依赖屏幕”既是使用体验，也是可检查的交付要求。

### 60 秒首用演练与构件放行卡 / 60-Second First-Use Exercise and Release Cards

在任何现场启动前，未来经授权的核验者可先用一套**不指向具体地点、不记录身份信息**的首用演练检查信息语言本身：让首次阅读者依次完成“选择模式、识别下一枚步行标记、在短停牌选择绕开或返回、在状态变化时选择改线、按共享返回规则结束”五个任务。每项只记录 **COMPLETE / INCOMPLETE / UNCERTAIN**，不收集姓名、人脸、轨迹、设备或行为画像；本提交包不预填任何测试结果。

![V5.2 Route Kit 的 60 秒首用演练：五个只靠实体信息完成的阅读任务，以及“读不清即 HOLD”的控制规则；不是现场测试结果、既有路线或设施声明](assets/figures/v5_2-route-kit-first-use-test.svg)

“能读懂”也不等于“已可运营”。每张构件放行卡仍须同时记录：对应的 KIT 编号、当前 `HOLD / RELEASED / REMOVED` 状态、所需的授权与现场证据、待指定的维护与停止接口、对使用者可见的下一步以及条件失效后的移除或降级动作。KIT-01—06 任一构件读不清、缺少适用放行证据、无法维护或没有人工停止权，均不得启用；一次首用演练通过也不证明合法通行、无障碍连续性、现场安全、服务、许可、实施或 Verified SLA 升级 [data:visual/assets/v5_2-noonline-route-kit.json#first_use_comprehension_test] [data:visual/assets/v5_2-noonline-route-kit.json#component_release_cards]。

## 1. 问题 / The Problem

AI 城市常把“会被算法看见”误当成“可以被人使用”。午间是最小却最诚实的检验窗口：短时间步行、热暴露、等待、问路、喝水、休息、过街和临时失效同时发生。若一条公共路径只能在手机、预测、预约或测试系统正常时才能成立，它不是可靠的城市服务。

本方案提出 **Noonline SLA**：先建立可读、可走、可停、可自主判断的实体公共空间，再让 AI 做预测、解释、提醒、维护辅助与动态调整。固定信息和空间次序应让使用者先自行完成路线；人工责任只在后台承担维护、例外处置和停止决定，而非成为日常使用的前置条件。AI 也不是公共服务的前置条件 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [standard:BARRIER-FREE-ENVIRONMENT-LAW]。
...
空间系统由 11 条概念 LineString、12 个主服务节点、8 个次级支持点和 5 个非数字判断/例外责任接口构成。它把三个不同原型连接为一条“研究 → 公共 → 市场 → 反馈”的可逆循环；这些接口不是五个日常值守岗位，也不构成任何服务人员已到位的承诺：
...
AI 原点不是 AI 展厅，而是让公众拥有选择权的城市知识客厅。公共入口采用无手机也可读的固定信息；N06 作为 **heritage interpretation point**，讲述京张的连接、维护和公共责任，不能被读作真实遗存；N05 组织坐、等、谈和非数字信息判断；AI 仅在侧向提供可选择的预测与解释，退出 AI 不等于退出空间。

最重要的空间动作是：

- 用实体知识线连接进入、阅读、停留、提问与离开，而非用屏幕替代场所。
- 将座椅、固定标识、非数字信息判断与可选 AI 拆开布置，避免“必须登录、等待服务人员或使用设备才可使用”。
- 将京张文化转译为可维护、可读、可被公众质询的日常信息系统，而不制造虚构遗产点。

![AI 原点公共知识客厅：实体知识线、停留、人工帮助与可选 AI](assets/figures/v4-figure-03-ai-origin.png)

**概念体验透视（非现场再现）。** 这张参与者生成的概念图说明实体知识线、可移动座椅、非数字信息判断与可选 AI 如何并置，而非确认任何既有建筑、遗产位置、入口、树荫或服务人员。启用时必须让人能够只依靠实体信息完成进入、阅读、停留、判断与离开；任何维护、清晰带或内容条件缺失，交互层和相应服务主张即撤除。例外协助若被设置，也只能作为后台责任接口，不能成为正常使用的必要步骤。[source:DESIGN-VISUAL-V4_5] [data:visual/assets/v4_5-reversible-prototype-atlas.json#PRA-02]
...
大钟寺的重点不是“把站口接到设计里”，而是把轨道到达后的第一分钟做成可读、可停、可自主判断的 **conceptual station-arrival interface**。真实背景限于大钟寺站、A/B 口以及公开的站内无障碍服务信息 [source:EXT-SRC-DAZHONGSI-STATION-202605]。方案从 N11 的方向判断、N09 的短暂停留与 F04 非数字判断/例外责任接口，走向 N10 的过街/无障碍决策点和 N12 的日常服务界面。
...
- 将高频商务午间转换转化为短停、方向判断、固定导向和自行改线的公共界面；例外责任只在后台处理，不要求日常沿线值守。

![大钟寺概念到达界面：真实站点背景与待核验的城市转换清晰分层](assets/figures/v4-figure-04-dazhongsi.png)

**概念体验透视（非现场再现）。** 图示只表达“到达标记—自主判断—短暂停留—公共清晰带—经核验替代路径或日常界面”的可逆关系，不确认真实车站出口、过街、商业、饮水、无障碍或营业状态。`P0-EXP-03` 只有在合法通行与无障碍替代被授权核验后才能演练；一旦证据失效，系统须改走已核验公共边界或暂停连续性主张。例外责任不等于现场常驻服务。[source:DESIGN-VISUAL-V4_5] [data:visual/assets/v4_5-reversible-prototype-atlas.json#PRA-03]
...
Noonline 的核心顺序仍是 **Physical → Human → Digital → AI**，但四层不等于四个日常使用步骤。对使用者，只有 Physical 层必须成立：固定导向、实体路线逻辑、停留预留与公共入口逻辑应支持自行进入、判断和离开。Human 层是后台的维护、例外处置与停止责任，不是沿线常驻陪同；Digital 和 AI 则是可退出的增强层。当 AI 正常，系统可预测拥挤、解释路线、提示服务状态并辅助维护；当 AI 降级，停止不可靠推荐；当 AI 关闭，路线的基本使用方式不应改变。

`AI_OFF_TEST = PASS_WITH_PROVISIONAL_PHYSICAL_NETWORK`。这表示概念网络具备 AI 关闭时以物理空间为主、以后台责任为例外的逻辑，不表示现有座椅、饮水、入口、过街或任何服务人员已经存在或真实运营 [data:visual/assets/noonline-sla-test-results.json#AI_OFF_TEST]。
...
方案面对的并非单一“AI 用户”，而是普通步行者、午间短时使用者、观察者、研发人员、轨道到达人，以及后台维护与例外责任角色。AI 在众智园可辅助测试状态解释，在 AI 原点可提供自愿的知识解释与反馈，在大钟寺可辅助动态提醒和维护；它不替代固定标识、公共清晰带、非数字判断或自行绕行。每个场景都保留退出 AI 的实体路径，异常时触发停止、降级、改线或后台例外处置。这样，AI+ 不是屏幕叠加，而是对城市服务可解释性、可维护性和可问责性的有限增强 [depth:ai_scenarios] [data:visual/assets/noonline-sla-test-results.json#AI_OFF_TEST]。
