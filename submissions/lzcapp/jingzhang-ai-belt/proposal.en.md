...
- **Title block** (bottom-right): map name, sheet No. `JZ-OV-01`, scale 1:8000, coordinate system CGCS2000, projection Gauss–Krüger 3° zone 117°E, vertical datum 1985 National Height Datum, design unit lzcapp (open-source, no qualification statement), drawing date 2026-08-30, version v3.3, security class public.
...
- **Font**: body and annotation use **Noto Sans SC (SIL OFL 1.1, open-source, commercially usable)**. A subset is **embedded directly in this package's HTML as a base64 data URI** (family `JZEmbeddedSC`, first in every `font-family` list), so offline rendering by the reviewer needs no system CJK font and issues no remote font request. Figure PNGs are rasterised from a local copy of the same typeface, so the glyphs are burned into the pixels. Neither the figures nor the HTML depend on any operating-system font. Open-source font license: [source:FIG-FONT-NOTO-CJK].

**Drawing version**: v3.3 (v1.9 re-renders `report/proposal.html` and `report/proposal.en.html` from the same canonical v3.3 bilingual source, removing the leftover stale version label, the system-font names and the stale self-assessment from the rendered output; the English `mobility-bluegreen` and `metrics-evidence` figures are now substantively equivalent to their Chinese counterparts, keeping only one explicitly labelled Chinese cross-reference subtitle; quantitative spatial and engineering wording in the key-area and implementation tables has been downgraded to concept test bands or options awaiting comparison, consistent with `metrics.json` listing FAR, building height and total floor area as unknown. See `figure_layout_compliance` in `metrics.json` and the v1.9 entry in `changelog.md`). The v1.0 original raster is at [source:FIG-OVERVIEW-V1].
...
**Building form & massing control**: Predominantly low-density garden-style R&D buildings, individual heights take a **concept test value of <=24 m**, dropping to <=12 m along the riverfront for view corridors — **not a control indicator**: `metrics.json` lists `building_height_m`, `floor_area_ratio` and total floor area as unknown, so this band is used only for massing comparison and view-corridor checking; its **applicability boundary** is the absence of official regulatory-plan, heritage and rail height-limit conditions, and it **requires** review by planning, heritage, rail and structural professions plus formal regulatory conditions before it can serve as a control basis. The formal language pairs "plinth + box" — the lower 2–3 floors host open exhibition and public-service platforms, with standardized R&D modules above. Retain-renovate-demolish strategy: retain valuable industrial heritage along the Qinghe (warehouse structures), convert low-efficiency industrial buildings into shared labs and standards workshops, demolish scattered structures without retention value [depth:height_massing_character], [depth:retain_renovate_demolish].
...
**Building form & massing control**: Medium-high density mixed use; heights take a **concept test value of 50–60 m** near the rail station and 24–35 m on the campus side to match the university scale — **not a control indicator and not an existing-condition finding**: `metrics.json` lists `building_height_m`, `floor_area_ratio` and total floor area as unknown, and it **requires** review by planning, heritage, rail and structural professions plus formal regulatory conditions before it can serve as a control basis. Retain-renovate-demolish classification (**concept list pending verification, not an existing-building inventory**: `geometry/buildings.geojson` carries concept footprints inside the design parcels, not a survey of existing buildings): it is proposed to retain the Tsinghua Garden station heritage building (heritage review prerequisite), convert low-efficiency commercial buildings along Wudaokou into open-source collaboration and achievement-release spaces, and build new talent apartments and mixed R&D-office buildings; the **17 `education` units come from the submitted geometry's design footprints and are not a count of existing buildings**, and are proposed for retention and upgrade as shared campus-enterprise innovation spaces [depth:height_massing_character], [depth:retain_renovate_demolish].
...
**Transport & slow-mobility organization**: Dazhongsi station is the central hub (ROAD-TC-2); the **concept option for comparison** for four-quadrant pedestrian connectivity is four crossing islands plus two underground passages — **pending comparison**: underground passages involve rail structures, utilities and property rights, the **applicability boundary** is that rail-station and utility-pipeline conditions are unconfirmed, and it **requires** rail, utility and transport professional review plus property rights and permits before engineering deepening; at-grade crossings with signal priority are an equivalent alternative. The southern vitality-belt section (ROAD-SPINE) connects to the station forecourt, running north through ROAD-NS-0 and ROAD-NS-1 (two north-south branch roads) to the central section. Vehicular traffic uses ROAD-EW-4; the station area has drop-off bays and centralized bicycle parking [data:geometry/roads.geojson#ROAD-SPINE], [depth:traffic_rail_slow_parking].
...
Key feasibility fields extracted from `renewal_projects.json`, covering cost estimates, implementation phases, effectiveness thresholds and risks for all six projects. All costs are benchmarked ranges (concept stage, not estimates/budgets), calibrated against comparable public projects; formal investment requires appraisal. **The "Key Effectiveness Threshold" column lists concept-stage test targets used for option comparison and self-evaluation, not committed values or assessed KPIs**; engineering wording about height, underground passages and connectivity rates are options awaiting comparison, and may serve as control or engineering bases only once official regulatory-plan, rail, utility and property conditions are confirmed and reviewed by the corresponding professions — consistent with `metrics.json` listing `building_height_m`, `floor_area_ratio` and total floor area as unknown.
...
| **JZ-04** Dazhongsi Station Four-Quadrant Pedestrian Connectivity | Station forecourt renovation 0.6–1.5 B CNY; underground passages & canopies 0.3–0.8 B CNY; annual O&M 5–10 M CNY/yr | 4 crossing islands + 2 underground passages **as concept options pending comparison / pre-check tasks** (not a construction commitment, 0–1 yr) | Four-quadrant all-weather pedestrian network, station-city core renewal (1–3 yrs) | Four-quadrant pedestrian connectivity >=95%; station forecourt opening rate >=95% (concept test targets, within 3 yrs) | Rail-station and utility-pipeline conditions pending |
...
The main proposal may be Chinese or English, with a full counterpart via `proposal.en.md` or `proposal.zh.md`; under `proposal_format_version` 2 a missing, incomplete, malformed or mismatched Chinese/English body text, text-bearing figure, HTML or A3/A0 counterpart is a **blocking package-readiness failure**, not a note; only legacy version 1 packages are reviewed under the compatibility rule that treats it as a non-blocking warning. A3/A0, HTML and text-bearing drawings should provide corresponding-language copies, preferring the event's recommended terminology. All images, drawings, icons, data and code assets must state source, license and authorization status in `sources.json` or `report/copyright_statement.md`. HTML pages must not load remote scripts, remote map tiles, remote fonts, iframes, forms or external APIs, and must not track reviewers.
...
**Source tiers and usability boundary**: `sources.json` registers 35 sources in two non-interchangeable tiers. ① **Maintainer-registered sources** (supplied `data/source_registry.json`): 9 in total — **7** formally usable, **1** background_only, **1** provisional_only; only the first 7 may serve as formal basis, while the background_only and provisional_only entries serve respectively as background reference and temporary generation boundary and must not be upgraded to formal basis. ② **Participant-registered external material**: 26 entries (public statistics, open-licence basemaps, prior-art references, etc.), each registered with source, purpose and limitation; these are **not in the maintainer registry and carry no formal usability**, serving only as background support for conceptual reasoning.
