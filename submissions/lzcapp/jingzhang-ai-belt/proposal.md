...
- **图签栏**（右下）：图名、图号 `JZ-OV-01`、比例尺 1:8000、坐标系 CGCS2000、投影 高斯-克吕格 3° 分带 117°E、高程基准 1985 国家高程基准、设计单位 lzcapp（开源，无资质声明）、成图日期 2026-08-30、版本 v3.3、密级 公开。
...
- [source:FIG-FONT-NOTO-CJK]：正文/标注主字体授权（Noto Sans SC，SIL OFL 1.1，开源可商用）；HTML 以 base64 data URI 子集内嵌该字体，不依赖系统字体、无远程字体请求。
...
- 当前登记摘要（以 supplied `data/source_registry.json` 为唯一口径）：共 9 条——正式可用 **7** 条、background_only **1** 条、provisional_only **1** 条；仅前 7 条可作正式依据，参与者自行登记的 26 条外部资料不自动取得维护者 formal 状态（见【来源分栏与可用性边界】）。
...
**建筑形态与体量控制**：以低密度花园式研发建筑为主，单体高度**概念测试取值 ≤24 m**、沿河界面 ≤12 m 以形成视线通廊——**非控制指标**：`metrics.json` 中 `building_height_m`、`floor_area_ratio`、总建筑面积均为 unknown，该区间仅用于体量比选与视线通廊校核；**适用边界**为缺少正式控规、文保与轨道限高条件，**须经**规划、文保、轨道、结构专业复核并取得正式控规条件后方可作为控制依据。建筑形态采用"平台+方盒"组合——底层 2-3 层为开放展示与公共服务平台，上部为标准研发单元模块。拆改留策略（**概念待核查清单，非现状建筑盘点**：`geometry/buildings.geojson` 表达的是设计分区内的概念基底，不是现状建筑调查）：建议保留有保留价值的工业遗存（清河沿线仓储建筑），改造低效工业厂房为共享实验室与标准工坊，拆除无保留价值的零散建筑 [depth:height_massing_character]、[depth:retain_renovate_demolish]。
...
**建筑形态与体量控制**：以中高密度混合利用为主，沿轨道站点周边**概念测试取值 50–60 m**、近校侧 24–35 m 以与校园尺度衔接——**非控制指标、非现状结论**：`metrics.json` 中 `building_height_m`、`floor_area_ratio` 与总建筑面积均为 unknown，**须经**规划、文保、轨道、结构专业复核并取得正式控规条件后方可作为控制依据。建筑拆改留分类（**概念待核查清单，非现状建筑盘点**：建筑基底为设计分区内的概念基底，不是现状建筑调查）：建议保留清华园火车站等历史建筑（文保核查前置），改造五道口沿线低效商业为开源协作与成果发布空间，新建人才公寓与研发办公混合楼宇；其中 `education` 类的 **17 个单元来自提交几何中的设计基底、不是现状建筑清点结果**，建议优先保留并升级为校企共享创新空间 [depth:height_massing_character]、[depth:retain_renovate_demolish]。
...
**建筑形态与体量控制**：以中高层混合利用为主，站城一体化核心区**概念测试取值 60–80 m**、外围 35–50 m 以与周边城市界面衔接——**非控制指标、非现状结论**：`metrics.json` 中 `building_height_m`、`floor_area_ratio` 与总建筑面积均为 unknown，**须经**规划、文保、轨道、结构专业复核并取得正式控规条件后方可作为控制依据。建筑形态强调"门户感"——大钟寺智能经济之门（LM-03）为标志性构筑，周边建筑以简洁体量与动态立面（LED 信息幕、智能玻璃）形成"智能经济街区"的整体意象。拆改留策略（**概念待核查清单，非现状建筑盘点**）：建议保留大钟寺文化资源周边建筑，改造低效商业与办公为智能终端展示与数据要素服务空间 [depth:height_massing_character]、[depth:retain_renovate_demolish]。
...
**交通与慢行组织**：大钟寺站为核心枢纽（ROAD-TC-2），四象限步行连通的**概念比选方案**为 4 处路口安全岛 + 2 处地下通道——**待比选**：地下通道涉及轨道结构、市政管线与权属，**适用边界**为轨道站点与市政管线条件未确认，**须经**轨道、市政、交通专业复核并取得权属与许可后方可进入工程深化；地面过街 + 信号优先为等效替代路径。京张遗址公园活力带南段（ROAD-SPINE）衔接大钟寺站前广场，向北经 ROAD-NS-0 与 ROAD-NS-1 两条南北向支路连接中段。机动车沿 ROAD-EW-4 组织，站前区域设即停即走泊位与非机动车集中停放区 [data:geometry/roads.geojson#ROAD-SPINE]、[depth:traffic_rail_slow_parking]。
...
以下从 `renewal_projects.json` 提取关键可行性字段，覆盖 6 个项目的成本估算、实施阶段、成效阈值与风险。所有成本为概念阶段粗估，非概算/预算，正式投资须经测算核定。**「关键成效阈值」列为概念测试阈值（design-stage test targets），用于方案比选与效果自评，非承诺值或考核指标**；表中涉及高度、地下通道、连通率的工程性表述均为待比选方案，须在官方控规、轨道、市政、权属条件确认并经相应专业复核后方可作为控制或工程依据——与 `metrics.json` 中 `building_height_m`、`floor_area_ratio`、总建筑面积为 unknown 的状态一致。

| 项目 | 概念成本估算（主要类别） | 近期交付 | 中期交付 | 关键成效阈值 | 核心风险 |
| --- | --- | --- | --- | --- | --- |
| **JZ-01** 京张遗址公园慢行断点缝合 | 慢行设施与铺装 0.3–0.8 亿元；桥下空间改造与照明 0.1–0.3 亿元；年度运维 300–600 万元/年 | 识别并缝合 3–5 处高优先断点，桥下空间示范段（0–1 年） | 遗址公园南北向慢行全线连通，接驳 5 处轨道/公交节点（1–3 年） | 慢行连通率 ≥95%（3 年内）；高优先断点缝合率 ≥95%（概念测试阈值，1 年内） | 跨环路节点协调难度大 |
| **JZ-02** 众智园清河创新界面 | 滨水生态修复与步道建设 0.5–1.2 亿元；展示设施与照明 0.2–0.5 亿元；年度运维 400–800 万元/年 | 清河滨水步道示范段 + 安全治理沙盒临时展场（0–1 年） | 800 m 创新交往走廊全线贯通，3 个主题节点建成（1–3 年） | 滨水步道连通率 ≥90%；展示节点开放率 ≥95%（概念测试阈值，3 年内） | 河道蓝线与防洪条件待确认 |
| **JZ-03** 原点社区近校成果转化街 | 首层业态更新与立面改造 0.4–0.9 亿元；公共空间与导视 0.1–0.3 亿元；年度运营 200–500 万元/年 | 开源发布厅场地整理与首层业态更新**作为待比选概念选项 / 前置核查任务**（非开工承诺，0–1 年） | 成果转化街整体更新，校区—园区慢行缝合完成（1–3 年） | 成果转化空间使用率 ≥80%；首层开放率 ≥90%（3 年内） | 校区边界与权属待确认 |
| **JZ-04** 大钟寺站四象限步行连通 | 站前广场改造 0.6–1.5 亿元；地下通道与连廊 0.3–0.8 亿元；年度运维 500–1000 万元/年 | 4 处路口安全岛 + 2 处地下通道**作为待比选概念选项 / 前置核查任务**（非开工承诺，0–1 年） | 四象限全天候步行网络建成，站城一体化核心区更新（1–3 年） | 四象限步行连通率 ≥95%；站前广场开放率 ≥95%（概念测试阈值，3 年内） | 轨道站点与市政管线条件待确认 |
...
| 中期更新（1–3 年） | 三处重点片区全线、大钟寺 PROV-KEY-003 四象限、JZ-04 步行连通 | 高校 + 产权方 + 轨道/交管协调 + 新基建运营 | 慢行连通率 ≥95%；滨水可达 ≥80%；成果转化 ≥30 项/年；入驻 ≥50 家；算力覆盖 ≥70%；四象限连通率 ≥95%（概念测试阈值） |
...
方案主文件可使用中文或英文，并应通过 `proposal.en.md` 或 `proposal.zh.md` 提供完整对照译文；在 `proposal_format_version` 2 下，中文/英文正文、含文字图件、HTML 及 A3/A0 对应件若缺失、不完整、畸形或映射错误，**均为阻断性包就绪失败**（blocking），而非提示；仅历史 version 1 包按兼容规则以 non-blocking warning 评审。A3/A0、HTML 和含文字图件也应提供对应语言副本，并优先使用 `docs/terminology-glossary.md` 的赛事推荐译法。所有图片、图纸、图标、数据和代码资产必须在 `sources.json` 或 `report/copyright_statement.md` 中说明来源、许可和授权状态。HTML 页面不得加载远程脚本、远程地图瓦片、远程字体、iframe、表单或外部 API，不得跟踪评审者行为。
