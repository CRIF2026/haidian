...
## P3-02｜评审响应：可读性修复、区域协同矩阵与长期运营链

本轮回应公开评审对 exact head `35b74be418` 的逐项意见：两个中文 HTML 在缺字体环境出现方框字（P0 阻断项）、区域协同、长期运营链、中英实质等价核对、E2→E3→E4 交接与空间剖面；Logo 锁定稿与触觉规范已由 P3-01 的 `BRAND-IDENTITY-01` 交付。本轮不改变任何空间图层、指标数值、来源等级或许可边界。

**P0｜离线中文字体可读性。** 评审环境缺少 CJK 字体时，`report/proposal.html` 与 `visual/index.html` 的中文渲染为方框。修复方式：把 SIL OFL 1.1 授权的 Noto Sans SC（Regular 与 Bold 两个字重）按本包实际用字子集化为 woff2（各约 230–240 KB），以 base64 data-URI 形式写入共享本地样式表 `visual/assets/fonts-noto-subset.css`，由中英四个 HTML 载体经 `<link>` 引入（投稿校验不允许字体文件扩展名，故不使用独立字体文件）；系统已装该字体时优先用本地副本（`local()` 优先），缺失环境自动回退到内嵌子集。等宽栈与既有系统字体栈保持不变。验证时在移除 `local()` 的模拟缺字副本上整页渲染复核：无方框、无裁切、无横向溢出；四份 PDF 早已内嵌各自字体子集，不受影响 [source:FONT-NOTO-SANS-SC]。

**P1｜区域协同矩阵（非承诺型）。** 评审指出方案的区域协同停留在内部三区两翼。下表补齐与北纬社区（海淀北部）、未来科学城、怀柔科学城、北京经开区及京津冀的接口想象，同时把“不声称”写清楚：没有接触、没有合作意向记录、没有资金或政府安排；所有接口都是可复用的模板产物，而不是已存在的关系。

| 区域 | 潜在能力互补 | 本包可复用接口产物 | 明确不声称 |
| --- | --- | --- | --- |
| 海淀北部（北纬社区方向） | 社区议题来源、数据劳动共同议定场景 | 数据票协议模板、无数据等价规则、品牌模板 | 未接触、无合作意向、无落地安排 |
| 未来科学城（昌平） | 能源与制造类测试场景 | 受控计算审计链、场景卡结构 | 同上；不暗示机构参与 |
| 怀柔科学城 | 大科学装置相关数据治理需求 | 派生链撤回回执与残余披露模板 | 同上；不暗示装置或平台对接 |
| 北京经开区 | 自动驾驶与智能制造测试生态 | 合成准入包络与 fail-closed 闸门模式 | 同上；不暗示测试区合作 |
| 京津冀 | 跨区域方法复制 | 全部模板与审计脚本的开源路径；本包许可为 `COMMUNITY-DISPLAY-ONLY`，跨区域复用需另行授权安排 | 同上；不把复制写成已发生 |

**P1｜长期运营链（agent.6 细化）。** 把原有月/季/半年/年度节律拆成可核验的运营链，每一环都写明角色、频次、证据产物、退出条件与公共回报验收；所有运营主体仍未指定，保持 `unknown`：

| 环节 | 角色（主体待确认） | 频次 | 证据产物 | 退出/停止条件 | 公共回报验收 |
| --- | --- | --- | --- | --- | --- |
| 议题桌与开发者社区 | 社区服务者、数据劳动者、技术团队（治理席位建议比例见正文，未法定化） | 每月 | 议题记录、反对意见与处理回执 | 参与者可随时撤回票据；不再开放新场景 | 年度结算厅核对（`SCN-10`） |
| 场景开放窗 | 场景使用方、独立复核 | 每季度 | 过闸清单、失败报告、停机记录 | 任一停止触发器触发即关闭窗口 | 失败报告与补救 SLA 公开 |
| 国际传播 | 投稿者与未来运营方（未确定） | 年度开放周（模板，未排期） | 双语一页纸、开放周导视模板（P3-01） | 无真实国际受众测试前不声称传播效果 | 不设置参与排名 |
| 线索转化 | 专业深化团队（未确定） | 门 0—门 3 逐门推进 | 每门的 go/no-go 记录 | 任一门条件不足即停在该门 | 转化不承诺资金或实施 |
| 品牌与标识维护 | 未来运营方（未确定） | 随版本修订 | `BRAND-IDENTITY-01` 审计输出 | 官方识别要求出现时让位 | 标识不用于诱导同意 |

**P1｜中英实质等价核对。** 本轮对 P3-01 与本节新增内容的中英版本逐项核对数值、证据等级、限制性表述与图位；确定性双语映射门禁（`bilingual_contract_version: "1"`）保持通过。该核对由维护 Agent 执行，不是独立人工复核；发现歧义时以两版共同传达的限制性较强含义为准，此原则已写入版权与合规说明。

**P2｜E2→E3→E4 交接表。** 下表把每个证据簇的当前状态、升级所需条件与禁止推断写成一行，供专业团队接手：

| 证据簇 | 当前 E2/E1 状态 | E3 需要（授权现场/用户） | E4 需要（专业/主体确认） | 当前取得状态 |
| --- | --- | --- | --- | --- |
| 场地边界与图层 | 临时几何复算一致（E2） | 授权踏勘与测绘 | 官方 polygon/控规文件 | 均未取得 |
| 数据票协议 | 双状态机+合成案例通过（E2） | 真实用户两线等价测试 | 法律与数据治理角色确认 | 均未取得 |
| 容量准入包络 | 抽象单元复算与负向拒绝（E2） | 实测到达率与服务时长 | 运营主体容量确认 | 均未取得 |
| 大钟寺演练模板 | 21/21 用例与 8/8 负向（E2） | 授权现场采集 | 场地运营与无障碍复核签署 | 均未取得 |
| 品牌识别系统 | 可复算规格与无障碍门槛（E2） | 用户识别/可用性测试 | 商标清权与专业品牌评审 | 均未取得 |
| 空间剖面与体验 | 概念剖面（E1） | 现状建筑普查与测绘 | 结构、消防、无障碍专业深化 | 均未取得 |

**P2｜合作站空间剖面。** 下图把一个治理站的“前区公共界面—中区共同工作—后区受控计算”剖面与两条正线的同一入口关系画出；它是概念表达，不是建筑方案，不含测绘、现状调查或结构/消防/无障碍结论。

![共数京张合作站空间剖面：前区公共界面、中区共同工作、后区受控计算，两条正线同一入口](assets/figures/station-section.png)

复现与边界：字体子集由本包文本实际用字生成，仅含包内用字；剖面图为原创绘制。以上所有新增内容均不产生 E3/E4/E5 证据，`SCN-06-E3E4` 继续 `DEFERRED / DO_NOT_CONTACT / DO_NOT_REASK`。
...
![核心空间指标与合成容量准入—拒绝决策图](assets/figures/metrics-evidence.png)
